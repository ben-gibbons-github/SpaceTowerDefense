...
{
    public class PlayerSuperRailBullet : Bullet
    {
        static Color ParticleColor = new Color(1, 0.25f, 0.25f);
        static float AttackLineWidth = 64;

        public override void Update(GameTime gameTime)
        {
            Vector2 StartPosition = Position.get();
            Vector2 EndPosition = Position.get() + Vector2.Normalize(Speed) * 1000;

            ParticleManager.CreateParticle(new Vector3(StartPosition.X, 0, StartPosition.Y), Vector3.Zero, ParticleColor, 100, 0);
            for (int i = 0; i < 100; i++)
                ParticleManager.CreateParticle(
                    ...
                    Vector3.Zero, ParticleColor, 50, 0);

            ParticleManager.CreateParticle(new Vector3(EndPosition.X, 0, EndPosition.Y), Vector3.Zero, ParticleColor, 100, 0);

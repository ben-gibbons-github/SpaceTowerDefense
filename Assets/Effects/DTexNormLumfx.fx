//Fields:
	...
	texture Luminance;
	sampler2D luminanceSampler = sampler_state {
		Texture = (Luminance);
		MinFilter = Linear;
		MagFilter = Linear;
		MIPFILTER = LINEAR;
	...
	float4 LuminanceColor = float4(1,1,1,1);
...
//Forward Technique
	...
	float4 ForwardPixelShaderFunction(ForwardVertexShaderOutput input) : COLOR0
	{
		...
		float4 LumColor = tex2D(luminanceSampler, input.UV); 
		...
		return LuminanceColor * LumColor + float4(LightColor.xyz,1) * (TextureColor * DrawColor + LightColor.w * SpecularColor);
